...
import {
  ...
  hasAdvancedChannelEditorValues,
...
type ChannelEditorNavItem = ReturnType<
  typeof buildChannelEditorNavigationState
>['items'][number] & {
  description?: string
  icon: ReactNode
}
...
function getSectionStatusIcon(
  status: ChannelEditorNavItem['status']
): ReactNode {
  if (status === 'error') {
              ...
              <div key={item.id}>
                ...
                >
                  ...
                  <span
                    ...
                    aria-label={item.statusLabelKey}
                  >
...
export function ChannelMutateDrawer({
  ...
  const editorNavigationState = useMemo(
    () =>
      buildChannelEditorNavigationState({
    ...
    [currentFormValues, form.formState.errors, isEditing]
  )

  const editorNavItems = useMemo<ChannelEditorNavItem[]>(
    () =>
      editorNavigationState.items.map((item) => {
        let icon = <Server className='h-4 w-4' aria-hidden='true' />
        if (item.id === CHANNEL_EDITOR_SECTION_IDS.credentials) {
          icon = <KeyRound className='h-4 w-4' aria-hidden='true' />
        } else if (item.id === CHANNEL_EDITOR_SECTION_IDS.models) {
          icon = <Boxes className='h-4 w-4' aria-hidden='true' />
        } else if (item.id === CHANNEL_EDITOR_SECTION_IDS.advanced) {
        ...
        return {
          ...
          description:
            item.id === CHANNEL_EDITOR_SECTION_IDS.advanced &&
            item.status !== 'error'
              ? undefined
              : t(item.statusLabelKey),
          statusLabelKey: t(item.statusLabelKey),
          icon,
          children: item.children?.map((child) => ({
            ...child,
            titleKey: t(child.titleKey),
          })),
        }
      }),
    [editorNavigationState.items, t]
  )

  const currentStatusLabel =
    CHANNEL_STATUS_LABELS[
      currentFormValues.status as keyof typeof CHANNEL_STATUS_LABELS
    ] || 'Unknown'
    ...
    if (isEditing && channelData?.data) {
      ...
      setAdvancedSettingsOpen(
        readAdvancedSettingsPreference() ||
          hasAdvancedChannelEditorValues(defaults)
      )
  ...
  const handleEditorNavNavigate = useCallback(
    (targetId: string) => {
      const isAdvancedTarget =
        ...
        (ADVANCED_SETTINGS_CHILD_SECTION_IDS as readonly string[]).includes(
          targetId
        )
              ...
              {isChannelDetailLoading ? (
                ...
                <div className='grid gap-5 lg:grid-cols-[13rem_minmax(0,1fr)] lg:items-start'>
                  <ChannelEditorNav
                    ...
                    progressLabel={editorNavigationState.progressLabel}
                          ...
                          <ChannelAuthSection>
                            ...
                            <FormField
                              ...
                              render={({ field }) => {
                                ...
                                const multiKeyModeLabel =
                                  keyMode === 'replace'
                                    ? t('replaced')
                                    : t('appended')
                                let keyDescription = t(FIELD_DESCRIPTIONS.KEY)
                                if (!isEditing && isBatchMode) {
                                  keyDescription = t(
