...
export function Search({ className = '', placeholder }: SearchProps) {
  ...
  return (
    <Button
      variant='outline'
      className={cn(
        'bg-muted/25 group text-muted-foreground hover:bg-accent relative size-8 shrink-0 justify-center rounded-md p-0 text-sm font-normal shadow-none sm:h-8 sm:w-40 sm:justify-start sm:px-2.5 sm:pe-12 md:flex-none lg:w-52 xl:w-64',
        className
      )}
    ...
    >
      <SearchIcon
        ...
        size={16}
      />
      <span className='hidden sm:ms-4 sm:inline'>{resolvedPlaceholder}</span>
