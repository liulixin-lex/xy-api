...
export const getPaymentAdminErrorMessage = (error, t, fallback) => {
  ...
  if (code === 'payment_limit_list_unavailable') {
    ...
    message = t(
      'Payment settings could not be saved. No changes were applied.',
    );
...
export const getRetainedCredentialDisableErrorMessage = (
  error,
  t,
  fallback,
) => {
...
export const getSubscriptionPlanAdminErrorMessage = (error, t) => {
  const code = getPaymentAdminErrorCode(error);
