...
} from './waffo-pancake-settings.js';
...
describe('Waffo Pancake settings request', () => {
  ...
  test('normalizes persisted environment values', () => {
    assert.equal(normalizeWaffoPancakeTestMode(true), true);
    assert.equal(normalizeWaffoPancakeTestMode('true'), true);
    assert.equal(normalizeWaffoPancakeTestMode(false), false);
    assert.equal(normalizeWaffoPancakeTestMode('false'), false);
    assert.equal(
      buildWaffoPancakeSavePayload({
        merchantID: '',
