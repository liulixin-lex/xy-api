...
func TestRequestProfileCanonicalWhitelistAndV1HashCompatibility(t *testing.T) {
	legacy, err := NewLegacyRequestProfile(
	...
	profile, err := NewRequestProfile(RequestProfileInput{
...
func TestRequestTrafficClassKeepsLegacyHashAndValidatesNewProfiles(t *testing.T) {
	legacy, err := NewLegacyRequestProfile("/v1/messages", "default", "claude-test", true, 0, 100, 20)
	require.NoError(t, err)
	legacyHash, err := legacy.Hash()
	require.NoError(t, err)
	legacyJSON, err := common.Marshal(legacy)
...
func TestClaudeCodeOnlyTrafficExclusionIsConsistentAcrossSelectors(t *testing.T) {
	...
	restricted := ChannelSnapshot{ID: 101, Status: common.ChannelStatusEnabled, TrafficClass: model.RoutingChannelTrafficClassClaudeCodeOnly}
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			profile, profileErr := NewLegacyRequestProfile("/v1/messages", "default", "claude-test", false, 0, 1, 1)
...
func TestRequestProfileRejectsNonCanonicalOrUnsafeValues(t *testing.T) {
	base := RequestProfileInput{
		GroupName:    "default",
		ModelName:    "gpt-test",
	...
	tests := []struct {
		name   string
		mutate func(*RequestProfileInput)
	}{
		{name: "unknown request kind", mutate: func(input *RequestProfileInput) { input.RequestKind = "custom" }},
		{name: "unknown source format", mutate: func(input *RequestProfileInput) { input.SourceFormat = "custom" }},
		{name: "unknown capability bit", mutate: func(input *RequestProfileInput) { input.RequiredCapabilities = RequestCapabilityMask(1 << 63) }},
		{name: "unknown modality bit", mutate: func(input *RequestProfileInput) { input.InputModalities = RequestModalityMask(1 << 15) }},
		{name: "negative quantity", mutate: func(input *RequestProfileInput) {
			input.InputTokens = RequestQuantity{State: RequestQuantityKnown, Value: -1}
		}},
		{name: "unknown quantity state", mutate: func(input *RequestProfileInput) { input.InputTokens = RequestQuantity{State: "estimated", Value: 1} }},
		{name: "hedge without retry", mutate: func(input *RequestProfileInput) { input.HedgeAllowed = true }},
		{name: "unsafe retry enabled", mutate: func(input *RequestProfileInput) {
			input.RetrySafety = RequestRetrySafetyUnsafe
			input.RetryAllowed = true
		}},
		{name: "stateful hedge", mutate: func(input *RequestProfileInput) {
			input.RequiredCapabilities = RequestCapabilityStateful
			input.RetryAllowed = true
			input.HedgeAllowed = true
		}},
		{name: "oversized region", mutate: func(input *RequestProfileInput) { input.Region = strings.Repeat("r", 65) }},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			input := base
			test.mutate(&input)
			_, err := NewRequestProfile(input)
...
func TestRequiredCapabilitiesFailClosedAcrossShadowAndBalanced(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			profile := requestProfileForCapabilityTest(t, test.kind, test.capability)
...
func TestReplayHardCapabilityExclusionSurvivesAffinityAndSoftFallback(t *testing.T) {
	...
	assert.Equal(t, DecisionAlgorithmShadow, shadowInput.AlgorithmVersion)
	...
	assert.Equal(t, DecisionAlgorithmBalanced, balancedInput.AlgorithmVersion)
...
func TestPlanBalancedAppliesCapabilityFilterBeforeAffinity(t *testing.T) {
	...
	session, err := NewRequestRoutingSession("balanced-capability", "default")
	require.NoError(t, err)
	...
	assert.Equal(t, DecisionAlgorithmBalanced, plan.AlgorithmVersion)
	assert.Equal(t, DecisionAlgorithmBalanced, plan.Replay.AlgorithmVersion)
...
func TestCaptureShadowReplayUsesCapabilityFilter(t *testing.T) {
	ResetSnapshotForTest()
	t.Cleanup(ResetSnapshotForTest)
	...
	SetSnapshotForTest(view)
	profile := requestProfileForCapabilityTest(t, RequestKindResponsesCompact, RequestCapabilityJSONSchema)
	profile.RequestPath = "/v1/responses/compact"
	input, active, err := CaptureShadowReplayRequest(ShadowRequest{
		RequestID: "shadow-capability", RequestPath: "/v1/responses/compact",
		GroupName: "default", ModelName: "gpt-test", Profile: &profile,
	})
	require.NoError(t, err)
	require.True(t, active)
	assert.Equal(t, DecisionAlgorithmShadow, input.AlgorithmVersion)
	...
	_, err = EnqueueDecision(DecisionInput{
		RequestID: "shadow-capability", PoolID: input.PoolID,
	...
	assert.Equal(t, DecisionAlgorithmShadow, audits[0].AlgorithmVersion)
...
func TestSnapshotModelPreResolvesUpstreamModelWithoutMutatingCapabilityEvidence(t *testing.T) {
	...
	configuration, err := model.NewDefaultRoutingChannelConfiguration(11, 1)
	require.NoError(t, err)
	view, invalid := snapshotModel(
		model.Channel{Id: 11, ModelMapping: &mapping}, configuration,
		11, nil, "default", "gpt-test", nil, false, time.Now().Unix(),
	)
...
func TestLegacyV1ReplayRemainsValidWithStructuredProfiles(t *testing.T) {
	input := shadowReplayInputForTest(t)
	input.AlgorithmVersion = DecisionAlgorithmShadowV1
	legacyHash, err := input.computeHash()
	require.NoError(t, err)
	input.SnapshotHash = legacyHash
...
func requestProfileForCapabilityTest(
	t *testing.T,
	kind RequestKind,
	capability RequestCapabilityMask,
) RequestProfile {
...
func TestCapabilityMaskValidationRejectsPartialKnownState(t *testing.T) {
	profile := requestProfileForCapabilityTest(
...
func TestRealtimeAndStatefulProfilesAreNeverHedgeEligible(t *testing.T) {
	for _, capability := range []RequestCapabilityMask{RequestCapabilityRealtime, RequestCapabilityStateful} {
		_, err := NewRequestProfile(RequestProfileInput{
