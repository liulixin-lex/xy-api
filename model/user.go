...
type User struct {
	...
	InviteRewardPercent           int                        `json:"invite_reward_percent" gorm:"type:int;default:-1;column:invite_reward_percent"`
...
func (user *User) TransferAffQuotaToQuota(quota int) error {
	...
	settledQuota, err := settleAvailableAffiliateRewardsTx(tx, user.Id, now)
	if err != nil {
		return err
	}
	...
	user.AffQuota -= quota
	user.Quota += quota
	if err := markTransferredAffiliateRewardsTx(tx, user.Id, quota, now); err != nil {
		return err
	}
...
func (user *User) Insert(inviterId int) error {
	if err := DB.Transaction(func(tx *gorm.DB) error {
		return withNormalizedEmailLock(tx, user.Email, func(tx *gorm.DB) error {
			...
			user.ApplyInviteRewardBinding(inviterId)
...
func (user *User) UpdateWithTx(tx *gorm.DB, updatePassword bool) error {
	...
	updates := map[string]interface{}{
		...
		"email":        newUser.Email,
		"github_id":    newUser.GitHubId,
		"discord_id":   newUser.DiscordId,
		"oidc_id":      newUser.OidcId,
		"wechat_id":    newUser.WeChatId,
		"telegram_id":  newUser.TelegramId,
		"linux_do_id":  newUser.LinuxDOId,
		"group":        newUser.Group,
