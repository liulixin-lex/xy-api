...
func TestReconcileLegacyRoutingTopologyPreservesStableIdentity(t *testing.T) {
	...
	require.Positive(t, first.TopologyEpoch)
	require.Len(t, first.TopologyHash, 64)
	...
	require.NoError(t, err)
	assert.Greater(t, second.TopologyEpoch, first.TopologyEpoch)
	assert.NotEqual(t, first.TopologyHash, second.TopologyHash)
	...
	reactivated, err := ReconcileLegacyRoutingTopologyContext(context.Background())
	require.NoError(t, err)
	assert.Greater(t, reactivated.TopologyEpoch, third.TopologyEpoch)
	assert.NotEqual(t, third.TopologyHash, reactivated.TopologyHash)
	credentials = loadRoutingCredentialsForTest(t, 101)
	...
	stable, err := ReconcileLegacyRoutingTopologyContext(context.Background())
	require.NoError(t, err)
	assert.Equal(t, reactivated.TopologyEpoch, stable.TopologyEpoch)
	assert.Equal(t, reactivated.TopologyHash, stable.TopologyHash)
