...
- [ ] **Step 1: 写 cause/display message 分离的失败测试**
...
- [ ] **Step 2: 写状态映射保留 source status 的失败测试**
...
- [ ] **Step 4: 实现 cause/source/display 三分离**
...
- [ ] **Step 1: 写 reliability 计数口径的失败测试**
...
- [ ] **Step 2: 写数据库合并与 selector neutral prior 的失败测试**
...
- [ ] **Step 5: 切换 Selector availability**
...
- [ ] **Step 1: 写 Capacity Hot Cache 的失败测试**
...
- [ ] **Step 4: 实现独立 Capacity Map 与冷却计算**
...
- [ ] **Step 7: 提交 Capacity Cooldown 核心**
...
- [ ] **Step 1: 反转 429/529 与 Retry-After 的 Breaker 测试**
...
- [ ] **Step 2: 写 semantic version 失败测试**
...
- [ ] **Step 5: 增加 Breaker semantic version 并过滤 hydrate**
...
- [ ] **Step 1: 写当前 attempt 真源与 metrics no-op 失败测试**
...
- [ ] **Step 2: 写数据面不消费 legacy index/aggregate 的失败测试**
...
- [ ] **Step 6: 让 Multi-Key Key 选择只看 operational state**
...
- [ ] **Step 7: 在 model/flush/refresh 阻止不可信状态**
...
- [ ] **Step 9: 提交 Multi-Key smart-state 安全降级**
...
- [ ] **Step 2: 写 origin task context 失败测试**
...
- [ ] **Step 6: 同步 origin task 的完整 credential context**
...
- [ ] **Step 8: 提交 raw-Key operational 状态修复**
...
- [ ] **Step 1: 写同一 classification 扇出的失败测试**
...
- [ ] **Step 2: 写 retry、stream 与内容安全失败测试**
...
- [ ] **Step 5: 统一分发 metrics、Breaker 与 Capacity**
...
- [ ] **Step 7: 让 violation normalize 保留证据**
...
- [ ] **Step 1: 写 Task code、local 与 idempotency 失败测试**
...
- [ ] **Step 2: 写 RelayTaskSubmit 本地前置错误与 Retry-After 失败测试**
...
- [ ] **Step 6: Task 每次 attempt 只分类一次且不自动跨渠道重试**
...
- [ ] **Step 7: 反转 Cost Connector serving health 测试**
...
- [ ] **Step 8: 删除 connector 对 serving AuthFailure 的写入**
...
- [ ] **Step 10: 提交 Task 与 Connector health 分离**
...
Run: gofmt -w types/error.go types/error_test.go dto/task.go pkg/routing_error/classifier.go pkg/routing_error/classifier_test.go model/routing_model.go model/routing_model_test.go model/channel.go model/channel_multikey_test.go pkg/routing_metrics/metrics.go pkg/routing_metrics/metrics_test.go pkg/routing_hotcache/cache.go pkg/routing_hotcache/cache_test.go pkg/routing_hotcache/capacity.go pkg/routing_hotcache/capacity_test.go pkg/routing_breaker/breaker.go pkg/routing_breaker/breaker_test.go service/error.go service/error_test.go service/channel.go service/channel_test.go service/violation_fee.go service/violation_fee_test.go service/channel_select.go service/channel_select_test.go service/routing/types.go service/routing/selector.go service/routing/selector_test.go setting/operation_setting/status_code_ranges.go setting/operation_setting/status_code_ranges_test.go middleware/distributor.go middleware/distributor_smart_routing_test.go relay/common/relay_info.go relay/common/relay_info_test.go relay/relay_task.go relay/relay_task_test.go controller/relay.go controller/relay_retry_test.go controller/channel.go controller/channel-test.go controller/channel_multikey_update_test.go controller/system_task_handlers.go controller/smart_routing_runtime_test.go controller/smart_routing_sub2api.go controller/smart_routing_task_test.go controller/smart_routing_test.go

Expected: exit 0。

Run: go vet ./types ./dto ./pkg/routing_error ./pkg/routing_metrics ./pkg/routing_hotcache ./pkg/routing_breaker ./service/routing ./service ./setting/operation_setting ./middleware ./relay/common ./relay ./controller ./model

Expected: exit 0。

- [ ] **Step 3: 执行新鲜后端、竞态与前端基线验证**

Run: go test ./... -count=1

Expected: PASS，0 failures。

Run: go test -race ./types ./pkg/routing_error ./pkg/routing_metrics ./pkg/routing_hotcache ./pkg/routing_breaker ./service/routing ./service ./middleware ./relay/common ./relay ./controller -count=1
