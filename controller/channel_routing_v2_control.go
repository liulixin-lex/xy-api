...
func SyncChannelRoutingCosts(c *gin.Context) {
	if err := decodeChannelRoutingEmptyControlBody(c.Request.Body); err != nil {
		writeChannelRoutingPolicyDraftError(c, http.StatusBadRequest, "invalid_cost_sync", "invalid channel routing cost sync request", err)
		return
	}
	identity, ok := requireChannelRoutingOperationIdempotency(c, model.RoutingOperationTypeCostSync, struct{}{})
	if !ok {
		return
	}
	operation, lookupErr := model.GetRoutingOperationByRequestIdentityContext(c.Request.Context(), identity)
	createdOperation := false
	if lookupErr == nil && operation.OperationType != model.RoutingOperationTypeCostSync {
	...
	if errors.Is(lookupErr, gorm.ErrRecordNotFound) {
		...
		spec := model.RoutingOperationSpec{
			Type: model.RoutingOperationTypeCostSync, EvaluationHash: identity.PayloadHash,
			...
			ActorID: common.GetContextKeyInt(c, constant.ContextKeyUserId), Reason: "manual routing cost sync",
			RequestKeyHash: identity.KeyHash, RequestPayloadHash: identity.PayloadHash,
		}
		operation, createdOperation, err = model.CreateRoutingOperationContext(c.Request.Context(), spec)
		if err != nil {
			writeChannelRoutingPolicyControlError(c, err)
			return
		}
	}
	if operation.Status == model.RoutingOperationStatusSucceeded ||
		operation.Status == model.RoutingOperationStatusFailed ||
		operation.Status == model.RoutingOperationStatusSuperseded {
	...
	task, createdTask, enqueueErr := service.EnqueueRoutingCostSyncOperation(c.Request.Context(), operation.ID)
	if enqueueErr != nil {
		message := common.SanitizeErrorMessage(enqueueErr.Error())
		if message == "" {
			message = "cost sync enqueue failed"
		}
		if failErr := model.FailUnattachedRoutingOperationContext(c.Request.Context(), operation.ID, errors.New(message)); failErr != nil {
			if errors.Is(failErr, model.ErrRoutingOperationClaimLost) {
				reloaded, reloadErr := model.GetRoutingOperationContext(c.Request.Context(), operation.ID)
				if reloadErr == nil && reloaded.SystemTaskID != "" {
					view, viewErr := channelRoutingOperationViewFromModel(reloaded)
					if viewErr != nil {
				...
				if reloadErr != nil {
					writeChannelRoutingPolicyControlError(c, reloadErr)
					return
				}
			} else {
				writeChannelRoutingPolicyControlError(c, failErr)
				return
			}
		}
		reloaded, reloadErr := model.GetRoutingOperationContext(c.Request.Context(), operation.ID)
		...
		c.JSON(http.StatusInternalServerError, gin.H{
			"success": false, "code": "cost_sync_enqueue_failed", "message": "failed to enqueue channel routing cost sync",
			"operation": operation,
		})
		return
	}
	operation, err := model.GetRoutingOperationContext(c.Request.Context(), operation.ID)
	if err != nil {
		writeChannelRoutingPolicyControlError(c, err)
		return
	}
	...
	if createdOperation {
		publishChannelRoutingControlEvent(channelrouting.RoutingEventTypeCostSyncQueued, operation.ExpectedRevision, gin.H{
			"operation_id": operation.ID, "system_task_id": task.TaskID,
		})
	}
	...
	if err := appendChannelRoutingOperationCreatedResult(&view, createdTask); err != nil {
		writeChannelRoutingPolicyControlError(c, err)
		return
	}
