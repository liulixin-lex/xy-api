...
func MigrateConsoleSetting(c *gin.Context) {
	...
	if err := model.DeleteOptions(oldKeys); err != nil {
		common.SysError("failed to remove legacy console options: " + err.Error())
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "迁移旧配置失败，请稍后重试"})
		return
	}
