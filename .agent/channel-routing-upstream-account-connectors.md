...
当前项目已经存在完整的 NewAPI / Sub2API 上游账号连接器，不需要从零创建模块。现有能力包括：
...
- 凭据与 binding：`model/routing_model.go`、`model/routing_credential_envelope.go`
- NewAPI 账号与价格：`controller/system_task_handlers.go`
- Sub2API JWT、账号与官方 DTO：`controller/smart_routing_sub2api.go`
- 成本快照归一化：`controller/system_task_handlers.go`、`model/routing_cost_v2.go`
- 成本来源 API：`controller/channel_routing_v2_cost_bindings.go`
- 正式前端：`web/default/src/features/channel-routing/costs/`

新增 provider 或升级协议时，应把官方 DTO 和转换保留在对应 connector 边界，不能把 provider 特有字段泄漏到通用选路逻辑。
